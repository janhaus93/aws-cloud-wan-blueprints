...
# Per-branch VPC CIDRs advertised into BGP. SDWAN hubs are not listed — they
# advertise only the loopback /32. See design.md §8.1.
BRANCH_VPC_CIDRS = {
...
""".format(loopback=loopback)
    ...
    # ---- Branch BGP network advertisements: VPC CIDR + test subnet CIDR ----
    # (Only the loopback /32 is advertised for SDWAN hubs.)
    branch_bgp_networks = ""
    if role == "branch":
        vpc_cidr = BRANCH_VPC_CIDRS[router_name]
        branch_bgp_networks += "set protocols bgp {asn} network {cidr}\n".format(
            asn=asn, cidr=vpc_cidr
        )
